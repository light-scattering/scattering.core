...
public class FAssemblyProducerTest {
    ...
    void produceCustom() {
        ...
        Assertions.assertAll("Validate FAssembly values",
                () -> assertTrue(resultA.asList().get(0).getRefCenter().isExact(1, 1, 1),
                        "The FAssembly A value is erroneous"),
                () -> assertTrue(resultB.asList().get(0).getRefCenter().isExact(2, 2, 2),
    ...
    void produceDistribution() {
        ...
        for (int i = 0 ; i < 1000 ; i++) {
            ...
            if (producer.produce().asList().get(0).getRadius() == 1) {
                countA++;
            } else {
                countB++;
            }
    ...
    void iterateAuto() {
        ...
        for (FAssembly<FSphere> fAssembly : producer.getList()) {
            ...
            if (fAssembly.asList().get(0).getRadius() == 1) {
                qCountA++;
            } else if (fAssembly.asList().get(0).getRadius() == 2) {
                aCountB++;
            } else if (fAssembly.asList().get(0).getRadius() == 3) {
                qCountC++;
            } else {
    ...
    void iterateRandom() {
        ...
        for (int i = 0 ; i < 20 ; i++) {
            if (results.get(i).asList().get(0).getRadius() != 1) {
                sequence = false;
                break;
            }
        }
    ...
    void produceWithEngine() {
        ...
        Assertions.assertAll("Validate FAssembly values",
                () -> assertEquals(1, resultA.asList().get(0).getMagnitude(),
                        epsilon, "The FPoint A magnitude is erroneous"),
                () -> assertEquals(1, resultB.asList().get(0).getMagnitude(),
