...
public class FRayHelperDef implements FRayHelper {
    ...
    private FRayHelperDef(GeometryFactory factory) {
    ...
    public boolean isProjectable(FVector origin, double x, double y, double z) {
        ...
        return isProjectable(origin, this.factory.getFPoint(x, y, z));
    }
    ...
    public boolean isProjectable(FVector origin, FPoint arg) {
        ...
        return getUnitDistance(origin, arg) > -1;
    }
    ...
    @Override
    public FPos3D shiftForward(FVector origin, double x, double y, double z, double distance) {
        FPoint fPoint = this.factory.getFPoint(x, y, z);

        shiftForward(origin, fPoint, distance);

        return fPoint.toFPos3D();
    }

    @Override
    public FPos3D shiftForward(FVector origin, FPos3D arg, double distance) {

        return shiftForward(origin, arg.getD0(), arg.getD1(), arg.getD2(), distance);
    }

    @Override
    public void shiftForward(FVector origin, FPoint in, double distance) {

        if (origin.isNearZeroLength()) {
            throw new IllegalStateException("The origin is a non-directional FVector");
        }

        shiftUnitForward(origin, in, distance);
    }

    @Override
    public void shiftForward(FVector origin, Geometry in, double distance) {
        ...
        in.toFPoints()
                .forEach(p -> shiftUnitForward(origin, p, distance));
    }

    @Override
    public FPos3D shiftBackward(FVector origin, double x, double y, double z, double distance) {
        FPoint fPoint = this.factory.getFPoint(x, y, z);

        shiftBackward(origin, fPoint, distance);

        return fPoint.toFPos3D();
    }

    @Override
    public FPos3D shiftBackward(FVector origin, FPos3D arg, double distance) {

        return shiftBackward(origin, arg.getD0(), arg.getD1(), arg.getD2(), distance);
    }

    @Override
    public void shiftBackward(FVector origin, FPoint in, double distance) {

        if (origin.isNearZeroLength()) {
            throw new IllegalStateException("The origin is a non-directional FVector");
        }

        shiftUnitBackward(origin, in, distance);
    }

    @Override
    public void shiftBackward(FVector origin, Geometry in, double distance) {
        ...
        in.toFPoints()
                .forEach(p -> shiftUnitBackward(origin, p, distance));
    }

    @Override
    public FPoint getFPointAtLength(FVector origin, double length) {
        ...
        var fPoint = this.factory.getFPoint();

        fPoint.set(origin.getRefHead());
        fPoint.subXYZ(origin.getRefBase());

        var tmp = length / origin.getMagnitude();

        fPoint.setX(origin.getRefBase().getX() + (fPoint.getX() * tmp));
        fPoint.setY(origin.getRefBase().getY() + (fPoint.getY() * tmp));
        fPoint.setZ(origin.getRefBase().getZ() + (fPoint.getZ() * tmp));

        return fPoint;
    }

    @Override
    public FPos3D getFPos3DAtLength(FVector origin, double length) {

        return getFPointAtLength(origin, length).toFPos3D();
    }

    // -------------------------------------------------------------------------------------------------

    private void shiftUnitForward(FVector origin, FPoint in, double dist) {

        if (dist < 0) {
            shiftUnitBackward(origin, in, -dist);
    ...
    private void shiftUnitBackward(FVector origin, FPoint in, double dist) {

        if (dist < 0) {
            shiftUnitForward(origin, in, -dist);
    ...
    private double getUnitDistance(FVector origin, FPoint arg) {
        ...
        boolean isValid = isUnitPartOfRay(origin, opX, opY, opZ);
    ...
    private boolean projectUnit(FVector origin, FPoint in) {
        ...
        boolean isValid = isUnitPartOfRay(origin, in.getX(), in.getY(), in.getZ());
    ...
    private boolean isUnitPartOf(FVector origin, FPoint arg) {
        double dist = getUnitDistance(origin, arg);
    ...
    private boolean isUnitPartOfRay(FVector origin, double x, double y, double z) {
