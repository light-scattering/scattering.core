...
public class GenerateHelper {
    ...
    public static int assemble(
            ...
            DistributionMixin dist,
            ExportMixin export,
            BiConsumer<FModelFactoryContext, FAggregate> builder)
    {
        ScatterFactory factory = ScatterCore.createFactory();
        String results = "";

        try {
            FAggregate template = DistributionService.assemble(factory, dist);

            builder.accept(factory.models(), template);
            ...
            results = ExportService.export(factory, template, export.format);
