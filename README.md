...
Note that the particles can also be analyzed separately:
```java
...
You can also evaluate the structural integrity and connectivity of the entire geometry:
```java
...
Note that these metrics heavily depend on the `epsilon` parameter. When a geometry is generated by a different aggregation algorithm or imported from an external source, this tolerance may need to be explicitly modified to match its specific definition of point contact and overlap.
