...
This section provides fundamental information about the aggregate's size, composition, and physical validity.
...
// Provides access to mean, median, percentiles, variance, skewness, kurtosis, etc.
FStat rp = fAggregate.getFStatParticleRadius();
```

Note that the particles can also be analyzed separately, for example:
```java
...
Note that these metrics heavily depend on the `epsilon` parameter. 
It defines the distance in arbitrary units between particles that can be considered in point contact. 
By default, it is set to `1E-4`. 
However, when the geometry is generated by a different aggregation algorithm, this tolerance may need to be modified.

The `epsilon` value can be applied to the entire aggregate or individually to each particle:

```java
// Apply to the entire aggregate
fAggregate.setParticleEpsilon(1E-4);

// Apply to individual particles separately
for (var fShape : fAggregate) {
    fShape.setEpsilon(1E-4);
}
```
...
FPos3D center = fAggregate.getMassCenter(massCenterType);
```

You can also retrieve any of these dynamically using the general `getCenter` method and the `Center` enum:
```java
