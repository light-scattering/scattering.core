...
public interface FRayHelper {
    ...
    FPos3D shiftForward(FVector origin, double x, double y, double z, double distance);
    FPos3D shiftForward(FVector origin, FPos3D arg, double distance);

    void shiftForward(FVector origin, FPoint in, double distance);
    void shiftForward(FVector origin, Geometry in, double distance);

    FPos3D shiftBackward(FVector origin, double x, double y, double z, double distance);
    FPos3D shiftBackward(FVector origin, FPos3D arg, double distance);

    void shiftBackward(FVector origin, FPoint in, double distance);
    void shiftBackward(FVector origin, Geometry in, double distance);

    //--------------------------------------------------

    FPoint getFPointAtLength(FVector origin, double length);
    FPos3D getFPos3DAtLength(FVector origin, double length);
}
