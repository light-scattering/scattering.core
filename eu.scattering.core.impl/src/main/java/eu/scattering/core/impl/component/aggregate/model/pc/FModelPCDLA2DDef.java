...
public class FModelPCDLA2DDef implements FModelPCDLA {
    ...
    public static FModelPCDLA create(FAggregate aggregate, ScatFactory factory) {
        ...
        return new FModelPCDLA2DDef(aggregate, factory);
    }
    ...
    private void initParticleB() {
        ...
        while (iterations++ < MAX_IT_INITIAL_ACCEPTOR) {
            particleB.setCenter(this.rndGen.nextDoubleOnCircle(particleA.getRadius() + particleB.getRadius()), 0);
    ...
    private void positionInternal(Shape particle) {
        ...
        do {
            particle.setCenter(this.rndGen.nextDoubleInCircle(this.rSpawn), 0);
            particle.getRefCenter().add(this.center);
