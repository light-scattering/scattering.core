...
public class FModelPCFilippov2DDef implements FModelPCTunable {
    ...
    private void initParticleB() {
        ...
        while (iterations++ < MAX_IT_INITIAL_ACCEPTOR) {
            particleB.setCenter(this.random.getFRand().nextDoubleOnCircle(particleA.getRadius() + particleB.getRadius()), 0);
    ...
    private boolean buildStep() {
        ...
        for (int i = 0; i < MAX_IT_SELECT; i++) {
            particle.setCenter(this.random.getFRand().nextDoubleOnCircle(radius), 0).translate(this.center);
